...
- supervised approach, and we can use the peak position 
- unsupervised is better for exploratory, is there clumping happening and this can gives us clues that something is prob not linear, how do these variables influence heirarchical clustering 
- Luclfideous distance,we've got one subgroup of people and how do we characterize, is there some evidence of a bias, and filtering
...
pairs plot, looking at some correlation (this was done), K means and hearcargcia distance, shape is important

what do those groups understand 

go a couple of different ways, are there any leading indicators, what is the smallest model I can use to get tik-tok virality 

*k means*

*heararchiam clustering *

some combination of number of week and what position they are on the chart, 
...
One supervised learning technique that we want to use is the logistic regression to estimate the probability of events occurring specific to our data set.
```{r}
#using tidy music data set, creating a logistic regression to predict the occurrance of a song entering the top 10
...
###each model's coefficents are statistically significant and have comparable AIC values.
#each slope value is relatively weak other than energy. This means that of the three predictors which are probabilities, songs with the highest 'energy' will be the best predictor of whether a song will make it onto the top 10 list
#next step, separate for training testing
```
